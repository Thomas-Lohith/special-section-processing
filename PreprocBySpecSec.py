...
def create_subdir(preproc_titles):
    ...
    for item in preproc_titles:
        ...
        #in order to use pathlib library  we need to convert the string into a Path object
        p_specsec =  Path("SpecSec")
        p_specsec_child = Path(f'SpecSec{ind+1}')

        p =  p_specsec / p_specsec_child
        Path(p).mkdir(parents=True)
                       #or 
        #Path('SpecSec'f'/SpecSec{ind+1}').mkdir(parents=True)

        with open(p/f'SpecSec{ind+1}_postproc.csv', 'w', newline='', encoding='utf-8') as csv_output:
        #with open(f'SpecSec/SpecSec{ind+1}/SpecSec{ind+1}_postproc.csv', 'w', newline='', encoding='utf-8') as csv_output:
